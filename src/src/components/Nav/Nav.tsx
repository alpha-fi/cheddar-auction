...
export function Nav() {
  const { tenk_contract, wallet, signIn, signOut } = useNear()
  const { auction_contract } = useAuctionNear();
  return (
    <nav className={css.nav}>
      <h1 className={css.title}>
        ...
        Tenk Contract: {tenk_contract ? `: ${tenk_contract}` : ''} <br/>
        Auction Contract: {auction_contract ? `${auction_contract}` : ''}
      </h1>
      {wallet && (
        wallet.getAccountId() ? (
          <Dropdown
