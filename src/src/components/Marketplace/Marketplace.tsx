...
export const Marketplace = () => {
    ...
    useEffect(() => {
        const getAuctions = async() => {
            ...
            const res = await Auction?.account.viewFunction(AUCTION_CONTRACT_ACCOUNT, "get_sales_by_nft_contract_id", args );
            if(res) {
                console.log(res);
    ...
    const getNFT = async(nftid: string) => {
        ...
        const res: Sale = await Tenk?.account.viewFunction(Tenk.contractId, "nft_token", args);
        return res;
    }
                                ...
                                <div className={css.nft_token}>
                                    ...
                                    <div className={css.nft_token_info}>
                                        <div style={{display: "flex"}}>
                                            <div>
                                                <b className="title">{nft.token.token_id}</b><br/>
                                                {timeLeft && <><b className="title">{timeLeft![index]}</b><br /></>}
                                            </div>
                                        </div>
                                        <div>
                                            {
