...
import { TimestampMs, AccountId, Balance, U128 } from "../tenk/index";
...
export class AuctionContract {
  constructor(
    ...
    public readonly nft_contract_id: string
  ) {}
  ...
  async create_auction(
    ...
    return providers.getTransactionLastResult(
      await this.create_auctionRaw(new_arg, options)
    );
  }
  ...
  create_auctionRaw(
    ...
    return this.account.functionCall({
      contractId: this.nft_contract_id,
  ...
  async offer(
    ...
    return providers.getTransactionLastResult(
      await this.offerRaw(args_new, options)
    );
  }
   ...
   */
  async offer_cheddar(
    args: { token_id: TokenId; amount: string },
    options: ChangeMethodOptions
  ): Promise<boolean> {
    const nft_contract_id = this.nft_contract_id;
    ...
    return providers.getTransactionLastResult(
      await this.offer_cheddarRaw(args_new, options)
    );
  }
  ...
  async accept_offer(
    ...
    return providers.getTransactionLastResult(
      await this.accept_offerRaw(args_new, options)
    );
  }
  ...
  async claim_nft(
    ...
    return providers.getTransactionLastResult(
      await this.claim_nftRaw(args_new, options)
    );
  }
  ...
  async remove_sale(
    ...
    return providers.getTransactionLastResult(
      await this.remove_saleRaw(args_new, options)
    );
  }
  ...
  get_sale(
    args: { nft_contract_token: string },
    options?: ViewFunctionOptions
  ): Promise<Sale> {
    return this.account.viewFunction(
      this.contractId,
  ...
  get_sales_by_nft_contract_id(
    args: { from_index: string; limit: u64 },
    options?: ViewFunctionOptions
  ): Promise<Sale[]> {
    const new_args = {
      nft_contract_id: this.nft_contract_id,
